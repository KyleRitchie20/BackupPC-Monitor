...
class GenerateBackupReport extends Command
{
     ...
     * @var string
     */
    protected $signature = 'backup:generate-report {--send : Send the report via email}';
     ...
     * @var string
     */
    protected $description = 'Generate and optionally email backup status reports';
    ...
    public function handle()
    {
        ...
        foreach ($sites as $site) {
            ...
            $pdf = $this->generatePdfForSite($site);
            $filename = "backup-report-{$site->name}-" . now()->format('Y-m-d') . '.pdf';
            ...
            if ($sendEmail) {
                $this->sendReportEmail($site, $path);
            }
        }
    ...
    /**
     * Generate PDF report for a specific site.
     */
    ...
    /**
     * Send report email to site admins.
     */
    protected function sendReportEmail(Site $site, $pdfPath)
    {
        // Get all admin users for this site
        $admins = $site->users()->whereHas('role', function ($query) {
            $query->where('name', 'admin');
        })->get();

        if ($admins->isEmpty()) {
            $this->warn("No admin users found for site: {$site->name}");
            return;
        }
        ...
        foreach ($admins as $admin) {
            ...
            try {
                Mail::send([], [], function ($message) use ($admin, $site, $pdfPath) {
                    $message->to($admin->email)
                        ->subject("Backup Report - {$site->name} - " . now()->format('Y-m-d'))
                        ->attach($pdfPath)
                        ->setBody("
                            <html>
                            <body>
                                <p>Hi {$admin->name},</p>
                                <p>Please find attached the weekly backup report for {$site->name}.</p>
