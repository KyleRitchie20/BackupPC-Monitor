                    ...
                    <form method="POST" action="{{ route('sites.update', $site) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $site->name)" required autofocus />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="backuppc_url" :value="__('BackupPC URL')" />
                                <x-text-input id="backuppc_url" class="block mt-1 w-full" type="url" name="backuppc_url" :value="old('backuppc_url', $site->backuppc_url)" required />
                            ...
                            <div>
                                ...
                                <select id="connection_method" name="connection_method" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                    <option value="ssh" {{ old('connection_method', $site->connection_method) == 'ssh' ? 'selected' : '' }}>SSH Tunnel</option>
                                    <option value="agent" {{ old('connection_method', $site->connection_method) == 'agent' ? 'selected' : '' }}>Polling Agent</option>
                                </select>
                        ...
                        <div id="ssh-settings" class="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="ssh_host" class="block mt-1 w-full" type="text" name="ssh_host" :value="old('ssh_host', $site->ssh_host)" />
                                    <x-input-error :messages="$errors->get('ssh_host')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="ssh_port" :value="__('SSH Port')" />
                                    <x-text-input id="ssh_port" class="block mt-1 w-full" type="number" name="ssh_port" :value="old('ssh_port', $site->ssh_port)" min="1" max="65535" />
                                    <x-input-error :messages="$errors->get('ssh_port')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="ssh_username" :value="__('SSH Username')" />
                                    <x-text-input id="ssh_username" class="block mt-1 w-full" type="text" name="ssh_username" :value="old('ssh_username', $site->ssh_username)" />
                                    <x-input-error :messages="$errors->get('ssh_username')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="ssh_password" :value="__('SSH Password (leave blank to keep current)')" />
                                    <x-text-input id="ssh_password" class="block mt-1 w-full" type="password" name="ssh_password" />
                        ...
                        <div id="agent-settings" class="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg" style="display: none;">
                            ...
                            <div>
                                <x-input-label for="api_key" :value="__('API Key (leave blank to keep current)')" />
                        ...
                        <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="backuppc_username" class="block mt-1 w-full" type="text" name="backuppc_username" :value="old('backuppc_username', $site->backuppc_username)" />
                                    <x-input-error :messages="$errors->get('backuppc_username')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="backuppc_password" :value="__('BackupPC Password (leave blank to keep current)')" />
                                    <x-text-input id="backuppc_password" class="block mt-1 w-full" type="password" name="backuppc_password" />
                        ...
                        <div class="mt-6">
                            ...
                            <input id="is_active" type="checkbox" name="is_active" value="1" {{ old('is_active', $site->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        </div>
